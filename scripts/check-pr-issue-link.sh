#!/usr/bin/env bash
# Require a GitHub issue reference on work PRs targeting sandbox.
#
# Closing keywords (Closes/Fixes/Resolves) only link/close when the PR targets
# the default branch (main). For PRs → sandbox we require Refs/#N (or bare #N).
#
# Usage (CI):
...
# Bypass:
#   - Dependabot actors
#   - PR base != sandbox (promote sandbox→main skipped here)
...
if [[ -n "${GITHUB_EVENT_PATH:-}" && -f "${GITHUB_EVENT_PATH}" ]]; then
  ...
  PR_JSON="$(gh api "repos/${GITHUB_REPOSITORY:-KleilsonSantos/VaultSpring}/pulls/${PR_NUMBER}")"
