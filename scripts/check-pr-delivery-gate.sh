...
#   PR_NUMBER=84 bash scripts/check-pr-delivery-gate.sh
#   PR_BASE=sandbox PR_HEAD=main bash scripts/check-pr-delivery-gate.sh
#   PR_BASE=sandbox PR_TITLE="feat: x" PR_BODY="Refs #83" PR_HEAD=feature/83-x bash scripts/check-pr-delivery-gate.sh
#
...
Full gate (Maven + delivery): bash scripts/pre-push-check.sh
EOF
exit 1
